...
public class KinoService {
    ...
    public void deleteKino() {
        ...
        List<ReservierungDTO> reservierungen = (List<ReservierungDTO>) commandProducer.sendCommandRequest(new CommandRequest(transactionId, CommandRequest.Operation.READ, "RESERVIERUNGSLISTE", null)).getEntity();
        List<AuffuehrungDTO> auffuehrungen = (List<AuffuehrungDTO>) commandProducer.sendCommandRequest(new CommandRequest(transactionId, CommandRequest.Operation.READ, "AUFFUEHRUNGSLISTE", null)).getEntity();
        List<FilmDTO> filme = (List<FilmDTO>) commandProducer.sendCommandRequest(new CommandRequest(transactionId, CommandRequest.Operation.READ, "FILMLISTE", null)).getEntity();
        List<KundeDTO> kunden = (List<KundeDTO>) commandProducer.sendCommandRequest(new CommandRequest(transactionId, CommandRequest.Operation.READ, "KUNDENLISTE", null)).getEntity();
        ...
        CommandResponse response = commandProducer.sendCommandRequest(new CommandRequest(transactionId, CommandRequest.Operation.DELETE, "KINO", kinoResponse.getEntity()));
